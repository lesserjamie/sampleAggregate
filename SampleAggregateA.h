...
static pthread_mutex_t workers_log_mutex;
...
class Master {
 public:
  ...
  void printLog() {
    std::cout << "Received samples from " << workers_log_.size();
    std::cout << " worker nodes." << std::endl;
    std::cout << "It took " << time_to_run_;
    std::cout << " seconds to run the most recent computation." << std::endl;

  }

 private:
  std::queue<int> workers_;                 // Worker socket file descriptors. 
  std::vector<int> workers_log_;            // Worker socket file descriptors of 
                                            //   all works seen during the 
                                            //   computation. For logging 
                                            //   purposes only.
  ...
  int initializeThreadpool() {
    ...
    workers_log_mutex = PTHREAD_MUTEX_INITIALIZER;
  ...
  int cleanThreadpool() {
    ...
    pthread_mutex_destroy(&workers_log_mutex);
  ...
  void* acceptIncomingWorkers(void) {
    ...
    while (is_running) {
      ...
      pthread_mutex_lock(&workers_log_mutex);
      if (workers_log_.size() == 0) {
	begin_time_ = clock();
      }

      if (std::find(workers_log_.begin(), workers_log_.end(), 
		   socketfd) == workers_log_.end()) {
	workers_log_.push_back(socketfd);
      }
      pthread_mutex_unlock(&workers_log_mutex);
...
class SampleAggregate {
 ...
 public:
  ...
  void printLog() {
    if (is_master_) return master_->printLog();
  }
